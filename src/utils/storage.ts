...
const STORAGE_KEYS = {
  ...
  ADMIN_AUTO_OPEN: 'zad_staff_admin_auto_open_v1',
  ACTIVE_VIEW: 'zad_staff_active_view_v1',
  MY_ORDERS: 'zad_staff_my_orders_v1',
};

export const DEFAULT_ADMIN_PASSWORD = 'zad2025';
...
export function resetStoredAdminPassword(): string {
  try {
...
export function saveStoredAdminAuth(isAuthenticated: boolean, remember = true) {
  try {
    if (isAuthenticated) {
      ...
      localStorage.removeItem(STORAGE_KEYS.ADMIN_AUTO_OPEN);
    }
  } catch (e) {
...
export function getStoredActiveView(): 'store' | 'admin' {
  try {
    const isAuth = getStoredAdminAuth();
    const autoOpen = localStorage.getItem(STORAGE_KEYS.ADMIN_AUTO_OPEN) === 'true';
    const lastView = localStorage.getItem(STORAGE_KEYS.ACTIVE_VIEW);
    if (isAuth && (autoOpen || lastView === 'admin')) {
      return 'admin';
    }
  } catch (e) {
    console.error('Error reading active view', e);
  }
  return 'store';
}

export function saveStoredActiveView(view: 'store' | 'admin') {
  try {
...
export function getStoredAdminAutoOpen(): boolean {
  try {
    return localStorage.getItem(STORAGE_KEYS.ADMIN_AUTO_OPEN) === 'true';
  } catch {
    return false;
  }
}

export function saveStoredAdminAutoOpen(enabled: boolean) {
  try {
    localStorage.setItem(STORAGE_KEYS.ADMIN_AUTO_OPEN, enabled ? 'true' : 'false');
  } catch (e) {
    console.error('Error saving admin auto open preference', e);
  }
}
