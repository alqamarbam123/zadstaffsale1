...
export default function App() {
  ...
  // Navigation & View
  const [isAdminLoggedIn, setIsAdminLoggedIn] = useState(() => getStoredAdminAuth());
  const [activeView, setActiveView] = useState<'store' | 'admin' | 'tracking'>(() => {
    const isAuth = getStoredAdminAuth();
    const autoOpen = getStoredAdminAutoOpen();
    const storedView = getStoredActiveView();
    if (isAuth && (autoOpen || storedView === 'admin')) {
      return 'admin';
    }
    return 'store';
  });
  ...
  const handleSelectView = (view: 'store' | 'admin' | 'tracking') => {
    ...
    setActiveView('store');
    saveStoredActiveView('store');
  };

  // Switch to store while keeping admin credentials active
  ...
  // Lock Admin (manual log out - requires password on next entry)
