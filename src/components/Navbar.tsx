            ...
            <button
              ...
              className={`relative flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all ${
                activeView === 'admin'
                  ? 'bg-slate-900 text-white shadow-xs'
                  : isAdminLoggedIn
                  ? 'text-slate-800 hover:text-slate-950 bg-emerald-50/90 hover:bg-emerald-100/90 border border-emerald-300/80 shadow-2xs'
                  : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'
              }`}
            >
              <ShieldCheck className={`w-3.5 h-3.5 ${isAdminLoggedIn ? 'text-emerald-600' : 'text-slate-400'}`} />
              <span>Admin Hub</span>
              {isAdminLoggedIn && activeView !== 'admin' && (
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" title="Admin logged in"></span>
              )}
              {pendingOrdersCount > 0 && (
          ...
          <button
            ...
            className={`font-bold py-2 px-1.5 rounded-xl transition-colors flex items-center justify-center gap-1 text-[11px] relative ${
              activeView === 'admin'
                ? 'bg-slate-900 text-white shadow-2xs'
                : isAdminLoggedIn
                ? 'text-slate-900 bg-emerald-50 border border-emerald-300/80 font-bold'
                : 'text-slate-600 bg-slate-50 hover:bg-slate-100'
            }`}
          >
            <ShieldCheck className={`w-3.5 h-3.5 ${isAdminLoggedIn ? 'text-emerald-600' : 'text-slate-400'}`} />
